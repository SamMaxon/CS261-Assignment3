...
int size(struct bst_node* root_node){
	if(root_node != NULL)
		return(size(root_node->left) + 1 + size(root_node->right)); 
	else 
		return 0;
}
...
int height(struct bst_node* root_node){
	int right; 
	int left; 
	if(root_node != NULL){
		right = height(root_node->right); 
		left = height(root_node->left); 
...
int path(struct bst_node* root_node, int sum){

	if(root_node != NULL){
		if(root_node->left == NULL && root_node->right == NULL && sum - root_node->val == 0)
			return 1; 
		return path(root_node->left, sum - root_node->val) || path(root_node->right, sum - root_node->val); 
	}
 ...
 */
void fill(struct bst_node* node, struct bst_iterator* bst_it){

	while(node != NULL){
		stack_push(bst_it->stack, node); 
		node = node->left; 
	}
}

struct bst_iterator* bst_iterator_create(struct bst* bst) {
	struct bst_iterator* bst_it = malloc(sizeof(struct bst_iterator));
	assert(bst_it); 
	bst_it->stack = stack_create(); 
	fill(bst->root, bst_it);
